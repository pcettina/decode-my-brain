...
def create_animated_raster_frames(
    spike_times: List[List[float]],
    neurons: NeuronPopulation,
    duration_ms: float,
    window_ms: float = 500.0,
    ...
    """
    Create an animated spike raster with Plotly animation frames.
    
    Args:
        spike_times: List of spike time lists per neuron
        neurons: NeuronPopulation object
        duration_ms: Total duration
        window_ms: Width of visible window
        step_ms: Time step between frames
        true_theta: True direction for highlighting
    ...
    # Create frames
    frames = []
    time_points = np.arange(window_ms, duration_ms + step_ms, step_ms)
    ...
    for t in time_points:
        ...
        for plot_idx, neuron_idx in enumerate(sort_idx):
            ...
            visible_spikes = [s for s in times if start_time <= s <= t]
    ...
    fig.update_layout(
        title="Animated Spike Raster",
        xaxis_title="Time (ms)",
        yaxis_title="Neuron",
        xaxis=dict(range=[0, window_ms + 10]),
        yaxis=dict(range=[-0.5, n_neurons - 0.5]),
        template='plotly_white',
        height=500,
        updatemenus=[{
            'type': 'buttons',
        ...
        sliders=[{
            ...
            'yanchor': 'top',
            'xanchor': 'left',
            'currentvalue': {'prefix': 'Time: ', 'suffix': ' ms', 'visible': True},
            'steps': [{'args': [[str(int(t))], {'frame': {'duration': 0, 'redraw': True},
                                                'mode': 'immediate'}],
                      'label': str(int(t)), 'method': 'animate'} 
                     for t in time_points]
        }]
    )
...
def plot_challenge_progress(
    ...
    """
    Create a progress display for challenge modes.
    
    Args:
        scores: List of scores/errors from completed trials
        target_score: Target to beat
    ...
    # Current score
    current_score = np.mean(scores) if scores else 0
    fig.add_trace(
        go.Indicator(
            mode='gauge+number',
            value=current_score,
            title={'text': 'Avg Error (°)'},
            gauge={
                'axis': {'range': [0, 90]},
                'bar': {'color': TRUE_COLOR if current_score < target_score else MODEL_COLOR},
                'threshold': {
    ...
    fig.add_trace(
        go.Indicator(
            ...
            number={'suffix': 's', 'font': {'size': 40, 'color': MODEL_COLOR if time_remaining < 10 else 'black'}}
        ),
        row=1, col=2
    )
...
def plot_achievement_badges(
    ...
    Args:
        achievements: List of achievement dicts with 'name', 'description', 'earned'
    ...
    cols = min(4, n_achievements)
    rows = (n_achievements + cols - 1) // cols
    
    fig = go.Figure()
    
    badge_colors = {
        True: '#f1c40f',   # Gold for earned
        False: '#bdc3c7'   # Gray for locked
    }
    ...
    for i, achievement in enumerate(achievements):
        ...
        fig.add_shape(
            type='circle',
            x0=x_pos - 0.4, y0=y_pos - 0.4,
            x1=x_pos + 0.4, y1=y_pos + 0.4,
            fillcolor=badge_colors[earned],
        ...
        fig.add_annotation(
            x=x_pos, y=y_pos - 0.6,
            text=achievement.get('name', 'Achievement'),
            showarrow=False,
            font=dict(size=10)
        )
    ...
    fig.update_layout(
        title="Achievements",
        showlegend=False,
        xaxis=dict(visible=False, range=[-0.5, cols * 1.2]),
        yaxis=dict(visible=False, range=[-rows * 1.2, 0.8], scaleanchor='x'),
        template='plotly_white',
        height=150 + rows * 100
    )
